...
## 更新摘要
**变更内容**
- 版本升级至1.0.6，包含依赖更新和功能改进
- 新增进程管理插件系统集成，包括 tauri-plugin-process 插件的安装、初始化和权限配置
- 完善权限架构说明，新增 process:default 权限配置
- Windows平台改进：使用 CREATE_NO_WINDOW 标志隐藏控制台窗口
...
**更新** 版本1.0.6引入了进程管理插件系统集成和权限架构完善，增强了应用的安全性和可控性。
...
- 权限配置：capabilities/default.json、gen/schemas/capabilities.json
...
B --> M["权限配置<br/>src-tauri/capabilities/default.json"]
B --> N["权限架构<br/>src-tauri/gen/schemas/capabilities.json"]
```

**图表来源**
- [lib.rs:13-90](file://src-tauri/src/lib.rs#L13-L90)
...
- [Cargo.toml:1-34](file://src-tauri/Cargo.toml#L1-L34)
- [capabilities/default.json:1-17](file://src-tauri/capabilities/default.json#L1-L17)
- [gen/schemas/capabilities.json:1-1](file://src-tauri/gen/schemas/capabilities.json#L1-L1)
...
- 进程管理插件：集成 tauri-plugin-process 插件，提供进程管理能力，包括退出和重启命令。
- 权限架构：基于能力系统的权限管理，通过 capabilities/default.json 配置默认权限集。
...
**更新** 新增进程管理插件系统集成，完善权限架构说明，提升了应用的安全性和可控性。
...
## 架构总览
下图展示从前端调用到平台命令执行的完整链路，以及错误传播路径。**更新** 新增进程管理插件和权限架构的处理流程。

```mermaid
sequenceDiagram
participant FE as "前端"
participant TAURI as "Tauri 运行时"
participant PROC as "进程管理插件"
...
**图表来源**
- [lib.rs:30-86](file://src-tauri/src/lib.rs#L30-L86)
...
#### 进程管理插件集成
- 依赖：新增 tauri-plugin-process = "2" 依赖。
- 功能：提供进程管理能力，包括退出和重启命令。
- 初始化：在 lib.rs 中通过 .plugin(tauri_plugin_process::init()) 注册插件。
- 权限配置：通过 capabilities/default.json 配置 process:default 权限。

**新增** 新增进程管理插件系统集成，提供进程控制能力。

**章节来源**
- [lib.rs:30](file://src-tauri/src/lib.rs#L30)
- [Cargo.toml:18](file://src-tauri/Cargo.toml#L18)
- [capabilities/default.json:14](file://src-tauri/capabilities/default.json#L14)

#### 权限架构说明
- 能力系统：基于 capabilities/default.json 配置默认权限集。
- 权限类型：process:default 提供进程管理权限。
- 权限验证：通过 gen/schemas/capabilities.json 进行权限验证。
- 插件权限：进程管理插件自动生成 exit.toml 和 restart.toml 权限文件。

**新增** 完善权限架构说明，新增进程管理权限配置。

**章节来源**
- [capabilities/default.json:1-17](file://src-tauri/capabilities/default.json#L1-L17)
- [gen/schemas/capabilities.json:1-1](file://src-tauri/gen/schemas/capabilities.json#L1-L1)
- [gen/schemas/desktop-schema.json:86-92](file://src-tauri/gen/schemas/desktop-schema.json#L86-L92)
...
graph LR
M["main.rs"] --> L["lib.rs"]
L --> PROC["进程管理插件<br/>tauri-plugin-process"]
...
L --> CT["Cargo.toml"]
L --> CAP["权限配置<br/>capabilities/default.json"]
L --> SCHEMA["权限架构<br/>gen/schemas/capabilities.json"]
```

**图表来源**
- [main.rs:4-6](file://src-tauri/src/main.rs#L4-L6)
- [lib.rs:1-90](file://src-tauri/src/lib.rs#L1-L90)
- [commands/mod.rs:1-4](file://src-tauri/src/commands/mod.rs#L1-L4)
...
**章节来源**
- [Cargo.toml:12-34](file://src-tauri/Cargo.toml#L12-L34)
- [tauri.conf.json:1-44](file://src-tauri/tauri.conf.json#L1-L44)
...
- **新增** 进程管理插件：提供进程控制能力，支持优雅退出和重启。
- **新增** 权限架构：基于能力系统的权限管理，提升安全性同时保持灵活性。
...
- **新增** 进程管理问题
  - 如果进程管理命令无效：检查 capabilities/default.json 中的 process:default 权限配置。
  - 如果插件初始化失败：确认 tauri-plugin-process 依赖版本与 Tauri 2.x 兼容。
- **新增** 权限架构问题
  - 如果权限验证失败：检查 gen/schemas/capabilities.json 中的权限配置。
  - 如果权限不生效：确认插件权限文件已正确生成（exit.toml、restart.toml）。
- **新增** 单实例问题
...
**更新** 新增了进程管理插件、权限架构和单实例插件相关的故障排除指南。
...
## 结论
本后端以清晰的分层与抽象实现了跨平台网络配置切换能力：命令层负责业务编排，模型层保证数据一致性，存储层提供可靠持久化，平台层屏蔽系统差异，错误层统一异常表达。**更新** 版本1.0.6引入了进程管理插件系统集成、权限架构完善、单实例插件集成、Windows平台的CREATE_NO_WINDOW标志改进、macOS应用激活增强和托盘菜单增强等功能，进一步提升了用户体验、安全性和系统兼容性。遵循本文最佳实践与故障排除建议，可在保证内存安全与性能的前提下快速迭代与扩展功能。
...
- **版本1.0.6变更**：
  ...
  - 新增进程管理插件依赖和配置（tauri-plugin-process）
  - 新增权限架构配置（capabilities/default.json）
  - 新增单实例插件依赖和配置
