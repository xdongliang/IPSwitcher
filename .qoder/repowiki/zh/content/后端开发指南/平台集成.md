...
- **新增**：CREATE_NO_WINDOW 标志
  ...
  - 在 netsh_command 函数中统一应用该标志，确保所有 Windows 平台的命令执行都隐藏控制台窗口。
...
- **新增**：NSApplication 激活机制
  ...
  - 在单实例插件回调中调用 activate_app() 函数，确保新实例能够正确显示并获取焦点。
...
- **新增**：Windows 平台的 CREATE_NO_WINDOW 标志
  ...
  - 确保权限检查过程不干扰用户界面体验。
  - 在 is_elevated() 函数中统一应用该标志，保证权限检测的静默执行。
...
- 应用入口配置：在 lib.rs 中注册单实例插件，设置回调函数处理重复启动场景。
...
- **新增**：Windows 平台的 CREATE_NO_WINDOW 标志优化
  ...
  - 避免多个控制台窗口堆积影响系统性能。
  - 在所有 Windows 平台的命令执行中统一应用该标志，确保一致性。
...
- **新增**：Windows 控制台窗口问题
  ...
  - 检查点：netsh_command 函数中的 creation_flags(CREATE_NO_WINDOW) 是否正确应用。
- **新增**：单实例冲突
  - 现象：启动第二个实例时应用程序无响应。
  - 处理：检查单实例插件配置，确认回调函数正确处理现有实例激活。
  - 检查点：lib.rs 中的单实例插件初始化和回调函数实现。
- **新增**：macOS 窗口激活问题
  - 现象：单实例激活后窗口未正确前置。
  - 处理：确认 NSApplication 激活机制正常工作，检查 macOS 权限设置。
  - 检查点：macos_activate.rs 中的 activate_app 函数和 objc2 依赖配置。
...
- 平台差异对比
  ...
  - **新增**：macOS 窗口激活：通过 objc2 调用 NSApplication 激活窗口。
- 兼容性与最佳实践
  ...
  - **新增**：macOS 窗口管理：利用 Accessory 激活策略和 NSApplication 激活机制，确保应用在后台也能正确显示。