...
- [src-tauri/src/admin.rs:26-164](file://src-tauri/src/admin.rs#L26-L164)
...
- 平台实现
  - Windows：使用 netsh 列举接口与配置，使用 netsh 设置静态 IP/DNS，使用 PowerShell 触发提升权限执行命令。
  - macOS：使用 networksetup 列举服务与配置，使用 networksetup 设置静态 IP/DNS，使用 osascript 触发提升权限执行命令。
- 权限与提权
  - 检测当前进程是否具备管理员/根权限。
...
## 架构总览
下图展示了从前端到后端、再到平台命令与系统工具的整体调用链路，以及权限提升的关键节点。
...
Plat->>Sys : 调用 netsh/networksetup
Sys-->>Plat : 返回执行结果
Plat-->>Tauri : 成功或错误
end
else 不需要管理员权限
...
Admin->>OS : powershell Start-Process ... -Verb RunAs
end
OS-->>Admin : 返回执行结果
Admin-->>Caller : 成功或错误
```
...
- 插件与运行时
  - Tauri 注册 shell 与 opener 插件，为命令执行与文件打开提供支持。
...
**章节来源**
- [src-tauri/src/admin.rs:26-164](file://src-tauri/src/admin.rs#L26-L164)
- [src-tauri/src/platform/windows.rs:95-144](file://src-tauri/src/platform/windows.rs#L95-L144)
- [src-tauri/src/platform/macos.rs:119-150](file://src-tauri/src/platform/macos.rs#L119-L150)
...
## 结论
IPSwitcher 通过清晰的平台抽象与工厂模式，将 Windows 与 macOS 的网络配置差异隐藏在统一接口之后。配合完善的权限检测与提权流程，实现了在不同平台上的稳定网络配置切换。前端通过 Tauri 命令与状态管理，提供了直观的用户交互与实时反馈。建议在后续迭代中进一步优化命令执行频率、增强错误恢复与回滚能力，并完善平台扩展点以支持更多网络服务管理场景。
