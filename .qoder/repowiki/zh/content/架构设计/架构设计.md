# 架构设计

<cite>
**本文档引用的文件**
- [package.json](file://package.json)
- [src-tauri/Cargo.toml](file://src-tauri/Cargo.toml)
- [src-tauri/tauri.conf.json](file://src-tauri/tauri.conf.json)
- [src/main.tsx](file://src/main.tsx)
- [src/App.tsx](file://src/App.tsx)
- [src/hooks/useProfiles.ts](file://src/hooks/useProfiles.ts)
- [src/hooks/useNetwork.ts](file://src/hooks/useNetwork.ts)
- [src/types/index.ts](file://src/types/index.ts)
- [src-tauri/src/lib.rs](file://src-tauri/src/lib.rs)
- [src-tauri/src/main.rs](file://src-tauri/src/main.rs)
- [src-tauri/src/commands/mod.rs](file://src-tauri/src/commands/mod.rs)
- [src-tauri/src/commands/profiles.rs](file://src-tauri/src/commands/profiles.rs)
- [src-tauri/src/commands/network.rs](file://src-tauri/src/commands/network.rs)
- [src-tauri/src/commands/interfaces.rs](file://src-tauri/src/commands/interfaces.rs)
- [src-tauri/src/storage/sqlite.rs](file://src-tauri/src/storage/sqlite.rs)
- [src-tauri/src/models/profile.rs](file://src-tauri/src/models/profile.rs)
- [src-tauri/src/platform/mod.rs](file://src-tauri/src/platform/mod.rs)
- [src/components/ProfileList.tsx](file://src/components/ProfileList.tsx)
</cite>

## 更新摘要
**所做更改**
- 更新了整体架构设计章节，反映项目已完成前后端分离架构重构
- 新增了模块化设计原则的详细说明
- 更新了核心组件分析，突出模块化架构的优势
- 完善了架构总览图，展示重构后的清晰分层结构
- 增强了依赖分析，体现模块间解耦的设计理念

## 目录
1. [引言](#引言)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [模块化设计原则](#模块化设计原则)
7. [依赖分析](#依赖分析)
8. [性能考量](#性能考量)
9. [故障排查指南](#故障排查指南)
10. [结论](#结论)
11. [附录](#附录)

## 引言
本架构文档面向IPSwitcher项目，系统性阐述其"前后端分离 + IPC通信 + 模块化设计"的整体架构。项目采用Tauri 2作为运行时，前端使用React + TypeScript，后端使用Rust，数据持久化采用SQLite。通过Tauri的IPC通道实现前端与后端的命令调用，结合平台抽象层实现跨平台网络配置管理（macOS/Windows）。本次重大架构重构建立了清晰的模块化设计原则，实现了前后端职责分离，提升了系统的可维护性和可扩展性。文档包含系统边界、组件交互关系、数据流向图、技术决策与权衡，并提供多类架构图示以帮助不同背景读者理解。

## 项目结构
项目采用典型的Tauri应用布局：前端位于src目录，后端位于src-tauri目录；前端通过@tauri-apps/api进行IPC调用，后端通过Tauri 2的invoke_handler暴露命令接口。构建流程由Vite负责前端打包，Tauri负责将前端产物嵌入原生壳并生成可执行程序。重构后的架构实现了严格的模块边界，每个模块都有明确的职责分工和清晰的接口定义。

```mermaid
graph TB
subgraph "前端(React)"
FE_Main["src/main.tsx"]
FE_App["src/App.tsx"]
FE_Hooks["src/hooks/*"]
FE_Components["src/components/*"]
FE_Types["src/types/index.ts"]
end
subgraph "后端(Tauri/Rust)"
BE_Main["src-tauri/src/main.rs"]
BE_Lib["src-tauri/src/lib.rs"]
BE_Commands["src-tauri/src/commands/*"]
BE_Storage["src-tauri/src/storage/*"]
BE_Models["src-tauri/src/models/*"]
BE_Platform["src-tauri/src/platform/*"]
end
subgraph "配置与构建"
Cfg_Pkg["package.json"]
Cfg_Cargo["src-tauri/Cargo.toml"]
Cfg_Tauri["src-tauri/tauri.conf.json"]
end
FE_Main --> FE_App
FE_App --> FE_Hooks
FE_App --> FE_Components
FE_App --> FE_Types
BE_Main --> BE_Lib
BE_Lib --> BE_Commands
BE_Lib --> BE_Storage
BE_Lib --> BE_Models
BE_Lib --> BE_Platform
Cfg_Pkg --> Cfg_Tauri
Cfg_Cargo --> Cfg_Tauri
```

**图表来源**
- [src/main.tsx:1-11](file://src/main.tsx#L1-L11)
- [src/App.tsx:1-208](file://src/App.tsx#L1-L208)
- [src-tauri/src/main.rs:1-7](file://src-tauri/src/main.rs#L1-L7)
- [src-tauri/src/lib.rs:1-71](file://src-tauri/src/lib.rs#L1-L71)
- [package.json:1-27](file://package.json#L1-L27)
- [src-tauri/Cargo.toml:1-30](file://src-tauri/Cargo.toml#L1-L30)
- [src-tauri/tauri.conf.json:1-48](file://src-tauri/tauri.conf.json#L1-L48)

**章节来源**
- [package.json:1-27](file://package.json#L1-L27)
- [src-tauri/Cargo.toml:1-30](file://src-tauri/Cargo.toml#L1-L30)
- [src-tauri/tauri.conf.json:1-48](file://src-tauri/tauri.conf.json#L1-L48)
- [src/main.tsx:1-11](file://src/main.tsx#L1-L11)
- [src-tauri/src/main.rs:1-7](file://src-tauri/src/main.rs#L1-L7)

## 核心组件
- 前端应用入口与路由容器
  - 应用入口：React根节点挂载于index.html的#root元素。
  - 主应用容器：App组件协调侧边栏列表、主内容区、状态栏，并监听托盘事件。
- 前端Hook与IPC封装
  - useProfiles：封装对后端profiles相关命令的invoke调用，统一错误与加载状态。
  - useNetwork：封装interfaces、current config、admin status与apply_profile等命令。
- 后端应用初始化与命令注册
  - lib.rs：初始化日志、数据库仓库、托盘图标、窗口事件；注册invoke_handler命令集合。
  - main.rs：平台级入口，隐藏控制台窗口（Windows发布版）。
- 数据模型与校验
  - models/profile.rs：定义Profile与IpMode，包含字段校验逻辑。
- 存储层
  - storage/sqlite.rs：基于rusqlite的ProfileRepository，支持跨平台数据库路径、迁移、CRUD。
- 平台抽象
  - platform/mod.rs：定义NetworkManager trait与当前网络配置结构体，按平台导出具体实现工厂函数。

**章节来源**
- [src/main.tsx:1-11](file://src/main.tsx#L1-L11)
- [src/App.tsx:1-208](file://src/App.tsx#L1-L208)
- [src/hooks/useProfiles.ts:1-131](file://src/hooks/useProfiles.ts#L1-L131)
- [src/hooks/useNetwork.ts:1-99](file://src/hooks/useNetwork.ts#L1-L99)
- [src-tauri/src/lib.rs:1-71](file://src-tauri/src/lib.rs#L1-L71)
- [src-tauri/src/main.rs:1-7](file://src-tauri/src/main.rs#L1-L7)
- [src-tauri/src/models/profile.rs](file://src-tauri/src/models/profile.rs)
- [src-tauri/src/storage/sqlite.rs:1-298](file://src-tauri/src/storage/sqlite.rs#L1-L298)
- [src-tauri/src/platform/mod.rs:1-64](file://src-tauri/src/platform/mod.rs#L1-L64)

## 架构总览
IPSwitcher采用"前端UI + 后端命令服务 + 跨平台平台层 + SQLite存储"的分层架构。前端通过@tauri-apps/api的invoke与后端建立IPC通道，后端根据命令分发到对应模块（profiles/interfaces/network），并调用平台层执行系统级网络配置操作，同时通过存储层持久化配置。重构后的架构实现了清晰的模块边界，每个模块职责单一，接口定义明确，便于维护和扩展。

```mermaid
graph TB
subgraph "前端"
UI_App["App 组件"]
UI_List["ProfileList 组件"]
Hook_Profiles["useProfiles Hook"]
Hook_Network["useNetwork Hook"]
end
subgraph "IPC"
IPC_Invoke["@tauri-apps/api/invoke"]
end
subgraph "后端"
Backend_Lib["lib.rs 初始化"]
Cmd_Profiles["commands/profiles/*"]
Cmd_Network["commands/network/*"]
Cmd_Interfaces["commands/interfaces/*"]
Storage_SQLite["storage/sqlite.rs"]
Platform_API["platform/mod.rs + 平台实现"]
Models_Profile["models/profile.rs"]
end
UI_App --> Hook_Profiles
UI_App --> Hook_Network
UI_List --> UI_App
Hook_Profiles --> IPC_Invoke
Hook_Network --> IPC_Invoke
IPC_Invoke --> Backend_Lib
Backend_Lib --> Cmd_Profiles
Backend_Lib --> Cmd_Network
Backend_Lib --> Cmd_Interfaces
Cmd_Profiles --> Storage_SQLite
Cmd_Network --> Platform_API
Cmd_Interfaces --> Platform_API
Storage_SQLite --> Models_Profile
```

**图表来源**
- [src/App.tsx:1-208](file://src/App.tsx#L1-L208)
- [src/components/ProfileList.tsx:1-55](file://src/components/ProfileList.tsx#L1-L55)
- [src/hooks/useProfiles.ts:1-131](file://src/hooks/useProfiles.ts#L1-L131)
- [src/hooks/useNetwork.ts:1-99](file://src/hooks/useNetwork.ts#L1-L99)
- [src-tauri/src/lib.rs:1-71](file://src-tauri/src/lib.rs#L1-L71)
- [src-tauri/src/commands/mod.rs:1-4](file://src-tauri/src/commands/mod.rs#L1-L4)
- [src-tauri/src/storage/sqlite.rs:1-298](file://src-tauri/src/storage/sqlite.rs#L1-L298)
- [src-tauri/src/platform/mod.rs:1-64](file://src-tauri/src/platform/mod.rs#L1-L64)
- [src-tauri/src/models/profile.rs](file://src-tauri/src/models/profile.rs)

## 详细组件分析

### 前端组件架构
- App容器
  - 管理选中/新建状态，协调ProfileList与ProfileForm，订阅托盘事件并触发应用流程。
- ProfileList
  - 渲染配置方案列表，支持新建、选择、切换动作。
- Hooks
  - useProfiles：封装列表、创建、更新、删除Profile的invoke调用，维护本地状态与错误。
  - useNetwork：封装接口枚举、当前配置查询、管理员权限检查、应用配置的invoke调用，并定时刷新当前配置。

```mermaid
classDiagram
class App {
+状态 : selectedId, isCreating
+方法 : handleSelectProfile(), handleNewProfile(), handleSave(), handleDelete(), handleSwitch(), handleConfirmApply()
+订阅 : tray-new-profile, tray-switch-profile
}
class ProfileList {
+属性 : profiles, selectedId, loading
+方法 : onSelect(), onSwitch(), onNew()
}
class useProfiles {
+方法 : fetchProfiles(), createProfile(), updateProfile(), deleteProfile()
+状态 : profiles[], loading, error
}
class useNetwork {
+方法 : fetchInterfaces(), fetchCurrentConfig(), checkAdmin(), applyProfile()
+状态 : interfaces[], currentConfig, isAdmin, loading, error
}
App --> ProfileList : "组合"
App --> useProfiles : "使用"
App --> useNetwork : "使用"
```

**图表来源**
- [src/App.tsx:1-208](file://src/App.tsx#L1-L208)
- [src/components/ProfileList.tsx:1-55](file://src/components/ProfileList.tsx#L1-L55)
- [src/hooks/useProfiles.ts:1-131](file://src/hooks/useProfiles.ts#L1-L131)
- [src/hooks/useNetwork.ts:1-99](file://src/hooks/useNetwork.ts#L1-L99)

**章节来源**
- [src/App.tsx:1-208](file://src/App.tsx#L1-L208)
- [src/components/ProfileList.tsx:1-55](file://src/components/ProfileList.tsx#L1-L55)
- [src/hooks/useProfiles.ts:1-131](file://src/hooks/useProfiles.ts#L1-L131)
- [src/hooks/useNetwork.ts:1-99](file://src/hooks/useNetwork.ts#L1-L99)

### Rust后端命令处理机制
- 初始化与生命周期
  - lib.rs在setup阶段构建托盘、设置激活策略、隐藏窗口关闭行为；通过invoke_handler集中注册命令。
- 命令模块
  - commands/mod.rs组织profiles、interfaces、network三大命令域。
- 数据访问与错误处理
  - storage/sqlite.rs提供ProfileRepository，封装CRUD、迁移、约束冲突处理。
  - models/profile.rs提供Profile结构与字段校验，确保输入合法性。
- 平台适配
  - platform/mod.rs定义NetworkManager trait，分别在macOS与Windows平台提供实现工厂函数，屏蔽系统差异。

```mermaid
sequenceDiagram
participant FE as "前端(Hook)"
participant IPC as "IPC(@tauri-apps/api)"
participant LIB as "lib.rs"
participant CMD as "commands/*"
participant ST as "storage/sqlite.rs"
participant PM as "platform/mod.rs"
FE->>IPC : invoke("list_profiles")
IPC->>LIB : 分发到invoke_handler
LIB->>CMD : 调用profiles : : list_profiles
CMD->>ST : 查询所有Profile
ST-->>CMD : 返回Profile数组
CMD-->>LIB : 序列化结果
LIB-->>IPC : 返回数据
IPC-->>FE : 更新状态
FE->>IPC : invoke("apply_profile", {profileId, interface})
IPC->>LIB : 分发到invoke_handler
LIB->>CMD : 调用network : : apply_profile
CMD->>PM : 执行平台特定配置应用
PM-->>CMD : 成功/失败
CMD-->>LIB : 返回结果
LIB-->>IPC : 返回结果
IPC-->>FE : 刷新当前配置
```

**图表来源**
- [src-tauri/src/lib.rs:1-71](file://src-tauri/src/lib.rs#L1-L71)
- [src-tauri/src/commands/mod.rs:1-4](file://src-tauri/src/commands/mod.rs#L1-L4)
- [src-tauri/src/storage/sqlite.rs:1-298](file://src-tauri/src/storage/sqlite.rs#L1-L298)
- [src-tauri/src/platform/mod.rs:1-64](file://src-tauri/src/platform/mod.rs#L1-L64)

**章节来源**
- [src-tauri/src/lib.rs:1-71](file://src-tauri/src/lib.rs#L1-L71)
- [src-tauri/src/commands/mod.rs:1-4](file://src-tauri/src/commands/mod.rs#L1-L4)
- [src-tauri/src/storage/sqlite.rs:1-298](file://src-tauri/src/storage/sqlite.rs#L1-L298)
- [src-tauri/src/platform/mod.rs:1-64](file://src-tauri/src/platform/mod.rs#L1-L64)

### SQLite数据存储层
- 数据库路径与迁移
  - 跨平台数据库路径：macOS位于用户应用支持目录，Windows位于APPDATA下的IPSwitcher目录。
  - 首次启动自动创建profiles表并执行迁移。
- 数据模型映射
  - Profile结构体与数据库字段映射，DNS服务器序列化为JSON字符串存储。
- 事务与并发
  - 使用Mutex保护连接，避免并发写入竞争；错误类型统一映射为AppError。

```mermaid
flowchart TD
Start(["初始化数据库"]) --> CheckDir["检查并创建数据目录"]
CheckDir --> OpenConn["打开/创建数据库连接"]
OpenConn --> Migrate["执行迁移: 创建profiles表"]
Migrate --> Ready(["数据库就绪"])
subgraph "CRUD操作"
Insert["insert(): 插入Profile<br/>JSON序列化DNS"]
Update["update(): 更新Profile<br/>JSON序列化DNS"]
Delete["delete(): 删除Profile"]
List["list_all()/get_by_id(): 查询Profile"]
end
Ready --> Insert
Ready --> Update
Ready --> Delete
Ready --> List
```

**图表来源**
- [src-tauri/src/storage/sqlite.rs:1-298](file://src-tauri/src/storage/sqlite.rs#L1-L298)

**章节来源**
- [src-tauri/src/storage/sqlite.rs:1-298](file://src-tauri/src/storage/sqlite.rs#L1-L298)

### Tauri集成与跨平台适配
- 集成方式
  - tauri.conf.json定义产品名、版本、窗口尺寸、托盘图标、构建参数（前端打包输出目录、开发URL）。
  - package.json脚本集成Vite与Tauri CLI，构建前先执行前端打包。
  - Cargo.toml声明Tauri 2、Shell与Opener插件、rusqlite(bundled)、平台目标依赖。
- 跨平台策略
  - platform/mod.rs通过cfg条件编译导出不同平台的NetworkManager实现，统一接口。
  - macOS使用Accessory激活策略隐藏Dock图标，符合菜单栏工具定位。
  - Windows发布版设置windows_subsystem为"windows"以隐藏控制台。

```mermaid
graph TB
Cfg["tauri.conf.json"]
Pkg["package.json"]
Cargo["Cargo.toml"]
MainRS["src-tauri/src/main.rs"]
LibRS["src-tauri/src/lib.rs"]
Plat["platform/mod.rs"]
Cfg --> LibRS
Pkg --> Cfg
Cargo --> LibRS
MainRS --> LibRS
LibRS --> Plat
```

**图表来源**
- [src-tauri/tauri.conf.json:1-48](file://src-tauri/tauri.conf.json#L1-L48)
- [package.json:1-27](file://package.json#L1-L27)
- [src-tauri/Cargo.toml:1-30](file://src-tauri/Cargo.toml#L1-L30)
- [src-tauri/src/main.rs:1-7](file://src-tauri/src/main.rs#L1-L7)
- [src-tauri/src/lib.rs:1-71](file://src-tauri/src/lib.rs#L1-L71)
- [src-tauri/src/platform/mod.rs:1-64](file://src-tauri/src/platform/mod.rs#L1-L64)

**章节来源**
- [src-tauri/tauri.conf.json:1-48](file://src-tauri/tauri.conf.json#L1-L48)
- [package.json:1-27](file://package.json#L1-L27)
- [src-tauri/Cargo.toml:1-30](file://src-tauri/Cargo.toml#L1-L30)
- [src-tauri/src/main.rs:1-7](file://src-tauri/src/main.rs#L1-L7)
- [src-tauri/src/lib.rs:1-71](file://src-tauri/src/lib.rs#L1-L71)
- [src-tauri/src/platform/mod.rs:1-64](file://src-tauri/src/platform/mod.rs#L1-L64)

## 模块化设计原则
IPSwitcher重构后的架构严格遵循模块化设计原则，实现了以下核心设计：

### 模块边界与职责分离
- **命令模块（commands）**：封装所有后端业务逻辑，按功能域划分profiles、interfaces、network三个子模块
- **存储模块（storage）**：专门负责数据持久化，当前实现为SQLite
- **平台模块（platform）**：抽象跨平台网络管理接口，隐藏系统差异
- **模型模块（models）**：定义数据结构和验证规则
- **前端模块（hooks/components）**：封装UI逻辑和用户交互

### 接口设计原则
- 每个模块对外暴露清晰的接口定义
- 内部实现细节对其他模块透明
- 通过trait和枚举实现类型安全的抽象
- 统一的错误处理机制和数据序列化标准

### 解耦与可测试性
- 模块间通过明确定义的接口通信
- 支持单元测试和集成测试
- 平台抽象层便于模拟测试
- 数据访问层支持内存数据库测试

**章节来源**
- [src-tauri/src/commands/mod.rs:1-4](file://src-tauri/src/commands/mod.rs#L1-L4)
- [src-tauri/src/storage/mod.rs:1-2](file://src-tauri/src/storage/mod.rs#L1-L2)
- [src-tauri/src/models/mod.rs:1-2](file://src-tauri/src/models/mod.rs#L1-L2)
- [src-tauri/src/platform/mod.rs:1-64](file://src-tauri/src/platform/mod.rs#L1-L64)

## 依赖分析
- 前端依赖
  - @tauri-apps/api用于invoke与事件监听；@tauri-apps/plugin-shell用于系统集成；React生态提供组件化能力。
- 后端依赖
  - tauri、tauri-plugin-shell、tauri-plugin-opener提供运行时与系统能力；rusqlite(bundled)提供SQLite访问；serde/serde_json用于序列化；uuid/chrono用于标识与时间戳；thiserror/env_logger用于错误与日志。
- 构建与打包
  - Vite负责前端开发与生产构建；Tauri将dist目录作为静态资源；tauri.conf.json指定构建命令与前端产物位置。

```mermaid
graph LR
FE["前端(React)"] --> API["@tauri-apps/api"]
FE --> Shell["@tauri-apps/plugin-shell"]
BE["后端(Rust)"] --> Tauri["tauri"]
BE --> ShellP["tauri-plugin-shell"]
BE --> Opn["tauri-plugin-opener"]
BE --> Sqlite["rusqlite(bundled)"]
BE --> Serde["serde/serde_json"]
BE --> UUID["uuid"]
BE --> Chrono["chrono"]
BE --> Log["log/env_logger"]
BE --> Err["thiserror"]
Build["构建"] --> Vite["Vite"]
Build --> TauriCLI["Tauri CLI"]
Vite --> Dist["dist"]
Dist --> TauriCLI
```

**图表来源**
- [package.json:1-27](file://package.json#L1-L27)
- [src-tauri/Cargo.toml:1-30](file://src-tauri/Cargo.toml#L1-L30)
- [src-tauri/tauri.conf.json:1-48](file://src-tauri/tauri.conf.json#L1-L48)

**章节来源**
- [package.json:1-27](file://package.json#L1-L27)
- [src-tauri/Cargo.toml:1-30](file://src-tauri/Cargo.toml#L1-L30)
- [src-tauri/tauri.conf.json:1-48](file://src-tauri/tauri.conf.json#L1-L48)

## 性能考量
- 前端
  - Hook缓存与状态最小化更新，避免不必要的重渲染；定时轮询当前配置频率适中（30秒），可根据实际场景调整。
- 后端
  - SQLite使用Mutex串行化访问，避免并发写入竞争；建议在高频写入场景引入队列或批处理优化。
  - 平台层调用系统API可能阻塞，建议在UI层显示加载态并限制并发操作。
- IPC
  - invoke调用为同步阻塞，建议批量请求与错误快速返回，减少等待时间。

## 故障排查指南
- 常见问题与定位
  - 数据库初始化失败：检查应用数据目录权限与路径解析（macOS/Windows）。
  - 网络配置应用失败：确认管理员权限、接口名称正确、平台实现可用。
  - 前端无法加载：确认Vite开发服务器已启动、tauri.conf.json中的devUrl与beforeDevCommand配置正确。
- 错误处理
  - 后端错误统一映射为AppError，前端Hook捕获并设置error状态；建议在UI层增加错误提示与重试按钮。
  - 日志：启用env_logger可在控制台查看初始化与命令执行日志。

**章节来源**
- [src-tauri/src/storage/sqlite.rs:1-298](file://src-tauri/src/storage/sqlite.rs#L1-L298)
- [src-tauri/src/lib.rs:1-71](file://src-tauri/src/lib.rs#L1-L71)
- [src/hooks/useProfiles.ts:1-131](file://src/hooks/useProfiles.ts#L1-L131)
- [src/hooks/useNetwork.ts:1-99](file://src/hooks/useNetwork.ts#L1-L99)

## 结论
IPSwitcher通过本次重大架构重构，成功建立了清晰的前后端分离架构和模块化设计原则。前端与后端职责明确分离，模块间通过清晰的接口通信，实现了高度的内聚和低耦合。Rust后端保证了系统级操作的安全与可靠性，SQLite提供了轻量且可靠的本地存储。平台抽象层使跨平台适配成为可能。整体架构在易用性、安全性与可扩展性之间取得平衡，为后续的功能扩展和维护奠定了坚实基础。

## 附录
- 关键数据模型
  - Profile：包含名称、IP模式、IP地址、子网掩码、网关、DNS服务器、接口名及时间戳。
  - NetworkInterface：接口名、显示名与是否活跃。
  - CurrentNetworkConfig：当前接口的IPv4配置与是否DHCP。

**章节来源**
- [src/types/index.ts:1-30](file://src/types/index.ts#L1-L30)
- [src-tauri/src/models/profile.rs](file://src-tauri/src/models/profile.rs)
- [src-tauri/src/platform/mod.rs:1-64](file://src-tauri/src/platform/mod.rs#L1-L64)