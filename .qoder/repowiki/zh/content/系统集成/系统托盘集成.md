...
## 更新摘要
**变更内容**
- 更新托盘菜单动态生成功能描述，包括基于配置文件列表的实时菜单更新机制
- 新增快速配置切换能力的技术实现细节
- 完善动态菜单重建触发条件和时机说明
- 增强跨平台兼容性的技术实现分析
...
**更新** 本版本重点介绍了托盘菜单的动态生成功能，包括基于配置文件列表的实时菜单更新和快速配置切换能力，以及完整的动态菜单重建机制。
...
E["commands/profiles.rs<br/>配置方案命令"]
F["platform/macos.rs<br/>macOS 网络管理"]
G["platform/windows.rs<br/>Windows 网络管理"]
end
subgraph "前端"
H["App.tsx<br/>事件监听与UI联动"]
end
A --> B
B --> C
B --> D
...
- 配置方案命令：处理配置方案的创建、更新、删除操作，并触发托盘菜单重建。

**更新** 新增配置方案命令组件，负责在配置变更时自动触发托盘菜单重建。
...
- [src-tauri/src/commands/profiles.rs:25-124](file://src-tauri/src/commands/profiles.rs#L25-L124)
...
**更新** 动态菜单项包含当前激活配置的复选标记，提供更直观的状态反馈。
...
### 快速配置切换功能
- 实时菜单更新：配置方案的创建、更新、删除操作会自动触发托盘菜单重建。
- 一键切换：用户无需打开主界面即可快速切换网络配置。
- 状态同步：托盘菜单显示当前激活的配置方案，提供清晰的状态反馈。

**更新** 新增快速配置切换功能的详细说明，包括实时更新机制和用户体验优化。

**章节来源**
- [src-tauri/src/commands/profiles.rs:25-124](file://src-tauri/src/commands/profiles.rs#L25-L124)
- [src-tauri/src/commands/network.rs:95-131](file://src-tauri/src/commands/network.rs#L95-L131)
...
- 配置方案命令模块依赖托盘模块进行菜单重建。

**更新** 新增配置方案命令与托盘模块的依赖关系。
...
Profiles["commands/profiles.rs"] --> Tray
Profiles --> Store
```
...
- 动态菜单未更新
  ...
  - 检查配置方案命令是否正确调用 `tray::rebuild_tray_menu`。
  - 参考路径：[动态菜单重建:92-135](file://src-tauri/src/tray.rs#L92-L135)
- 快速切换功能失效
  - 确认配置方案命令中正确调用了菜单重建函数。
  - 检查前端事件监听是否正常工作。
  - 参考路径：[配置方案命令:25-124](file://src-tauri/src/commands/profiles.rs#L25-L124)
- 应用配置失败（权限不足）
...
## 结论
该托盘集成方案通过后端统一构建与事件处理、前端事件驱动 UI 的方式，实现了跨平台的稳定体验。动态菜单重建与图标模板设置提升了可维护性与视觉一致性；结合平台特定的网络命令实现，满足了不同系统的差异化需求。快速配置切换功能提供了便捷的用户体验，用户无需打开主界面即可进行网络配置切换。建议在实际部署中关注权限与图标资源的配置，并在方案变更时及时重建菜单以保证用户体验。

**更新** 新增快速配置切换功能的用户体验优势说明。
...
- 最佳实践
  ...
  - 动态更新：确保配置变更时及时触发菜单重建，保持 UI 与数据的一致性。
- 参考路径
  ...
  - [配置方案命令:25-124](file://src-tauri/src/commands/profiles.rs#L25-L124)
