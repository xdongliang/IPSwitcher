...
- **改进macOS平台特性**：通过 Info.plist 和 macOS Activate 模块实现更好的窗口激活和焦点管理，**新增** 权限提升后自动激活应用功能
...
**更新** 本次更新重点介绍了单实例控制的集成，通过 tauri-plugin-single-instance 插件防止多个应用程序实例同时运行，支持单实例唤醒和窗口焦点管理。同时增强了系统托盘功能，包括动态菜单生成、快速配置切换和菜单重建机制，以及改进的Windows和macOS平台特定功能。**新增** macOS 平台通过专用的 `macos_activate.rs` 模块实现了权限提升后的自动应用激活功能，确保用户在权限提升后能够立即看到应用窗口。

## 项目结构
后端采用 Tauri v2 Rust 应用，前端通过 Vite 构建并通过 Tauri 暴露命令与事件。系统托盘在应用启动时初始化；网络配置通过平台抽象层调用系统命令；管理员权限检查与提升在需要时触发；配置数据持久化使用 SQLite；单实例控制通过专用插件实现；**新增** macOS 平台通过 `macos_activate.rs` 模块提供精确的窗口激活控制。
...
- **macOS 应用激活**：**新增** 通过 `macos_activate.rs` 模块提供精确的窗口激活控制，确保权限提升后应用能够自动激活并显示主窗口。
...
## 架构总览
系统采用"前端（Vite）+ 后端（Tauri/Rust）"双层结构。后端负责系统级操作（托盘、权限、网络配置、单实例控制），并通过命令与事件与前端通信。**新增** macOS 平台通过专用激活模块确保权限提升后的应用状态管理。
...
- **窗口唤醒机制**
  ...
  - **更新** macOS 平台使用 `macos_activate::activate_app()` 确保窗口获得焦点。
...
participant Activate as "macOS 激活模块"
participant Window as "主窗口"
NewInstance->>SingleInstance : 启动应用
SingleInstance->>ExistingInstance : 检测到现有实例
ExistingInstance->>Activate : 调用 activate_app()
Activate->>Window : 激活应用窗口
...
Exec --> Success{"执行成功？"}
Success --> |是| Activate["调用 macOS 激活模块"]
Activate --> Apply
...
### macOS 应用激活功能与窗口焦点管理
- **激活模块实现**
  - **新增** `macos_activate.rs` 模块提供精确的窗口激活控制，使用 Objective-C 运行时调用 `NSApplication` 的 `activateIgnoringOtherApps` 方法。
  - 通过 `activateIgnoringOtherApps: true` 参数确保应用窗口在前台显示，即使应用设置为后台运行模式。
  - 模块包含条件编译，非 macOS 平台提供空操作实现，确保跨平台兼容性。
- **权限提升后的自动激活**
  - **新增** 在权限提升流程完成后，自动调用 `activate_app()` 确保用户能够立即看到应用窗口。
  - 与托盘菜单事件和单实例唤醒机制协同工作，提供无缝的用户体验。
- **系统集成配置**
  - **新增** 在应用启动时设置 `ActivationPolicy::Accessory`，使应用以辅助工具模式运行，隐藏 Dock 图标。
  - 配合 `Info.plist` 中的 `LSUIElement=true` 设置，实现完全的后台运行场景。
- **窗口管理机制**
  - 与 Tauri 的窗口管理机制深度集成，确保激活后窗口能够正确显示和获得焦点。
  - 支持多种触发场景：托盘菜单点击、单实例唤醒、权限提升后自动激活。
...
User->>Admin : 触发权限提升
Admin->>Admin : 执行提权命令
Admin->>Activate : 调用 activate_app()
Activate->>Window : NSApplication.activateIgnoringOtherApps(true)
Activate->>Window : 显示窗口
Activate->>Window : 设置焦点
Window-->>User : 应用窗口激活完成
```

**图表来源**
- [src-tauri/src/macos_activate.rs:10-15](file://src-tauri/src/macos_activate.rs#L10-L15)
- [src-tauri/src/lib.rs:54-62](file://src-tauri/src/lib.rs#L54-L62)
- [src-tauri/src/tray.rs:16-22](file://src-tauri/src/tray.rs#L16-22)

**章节来源**
- [src-tauri/src/macos_activate.rs:1-24](file://src-tauri/src/macos_activate.rs#L1-L24)
- [src-tauri/src/lib.rs:32-38](file://src-tauri/src/lib.rs#L32-L38)
- [src-tauri/src/lib.rs:54-62](file://src-tauri/src/lib.rs#L54-L62)
- [src-tauri/src/tray.rs:16-22](file://src-tauri/src/tray.rs#L16-L22)
...
- **外部依赖**
  ...
  - **新增** macOS 平台依赖 Objective-C 运行时进行窗口激活。
...
Lib --> Activate["macos_activate.rs<br/>窗口激活"]
...
- **性能考量**
  ...
  - **新增** macOS 激活模块提供即时的窗口焦点管理，响应速度快。
- **可靠性保障**
  ...
  - **新增** macOS 激活模块提供稳定的窗口焦点管理，避免权限提升后的用户体验问题。
...
- **macOS 应用激活问题**
  - **新增** 检查 `macos_activate.rs` 模块是否正确导入到 `lib.rs` 中。
  - **新增** 验证 Objective-C 运行时调用是否正常，检查 `NSApplication` 类是否可访问。
  - **新增** 确认 `activateIgnoringOtherApps: true` 调用是否成功执行。
  - **新增** 验证权限提升后是否正确调用了 `activate_app()` 函数。
- **配置应用后网络未生效**
...
- **macOS 平台问题**
  ...
  - **新增** 验证 macOS 版本是否满足最低系统要求（10.15）。
...
## 结论
该系统通过 Tauri 将前端与系统能力整合，实现了稳定的托盘集成、灵活的权限管理、跨平台网络配置和可靠的单实例控制。**更新** 本次重大增强包括单实例控制集成，通过 tauri-plugin-single-instance 插件防止多个应用程序实例同时运行，支持单实例唤醒和窗口焦点管理。**新增** macOS 平台通过专用的 `macos_activate.rs` 模块实现了权限提升后的自动应用激活功能，确保用户在权限提升后能够立即看到应用窗口。**新增** Windows 平台的管理员权限管理功能通过应用程序清单文件请求管理员权限，显著改善了用户体验。托盘菜单与事件驱动前端交互，平台抽象层屏蔽差异，权限模块在必要时触发提升流程，存储模块保障配置持久化，单实例控制确保应用稳定性。整体架构清晰、职责分离，具备良好的扩展性与可维护性。
...
- **macOS 特定配置**
  ...
  - **新增** macOS 激活模块确保权限提升后的应用状态管理。
- **单实例控制**
  - **新增** tauri-plugin-single-instance 插件提供可靠的单实例管理。
  - **新增** 支持自定义唤醒回调，实现窗口显示和焦点管理。
  - **新增** 跨平台兼容的单实例控制机制。
- **权限提升与激活集成**
  - **新增** macOS 权限提升后自动调用激活模块，确保用户体验连贯性。
  - **新增** Windows 权限提升通过清单文件和 PowerShell 实现，减少前台干扰。
