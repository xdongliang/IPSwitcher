...
## 更新摘要
**所做更改**
- 更新版本号至1.0.4，反映最新的版本升级
- 增强GitHub Actions工作流中的发布说明，添加平台特定的安装指导
- 新增自动更新系统集成，支持应用内更新检查和下载安装
- 更新CI/CD工作流配置，增加代码签名密钥支持
- 增强前端更新检查组件，提供用户友好的更新界面
- 优化依赖管理，新增更新插件和进程管理插件
- 增强打包配置，支持自动更新产物生成
...
## 简介
本指南面向IPSwitcher项目的部署与发布，覆盖前端构建配置、Rust后端编译、跨平台打包以及发布准备等全流程。内容基于仓库中的实际配置文件，确保可操作性和准确性。特别关注版本1.0.4的更新，包括增强的GitHub Actions工作流、平台特定的安装指导、新增的自动更新系统、改进的CI/CD工作流和增强的用户界面体验。

## 项目结构
项目采用前后端分离架构：前端使用Vite+React，后端使用Tauri 2（Rust）。前端通过Vite进行开发与构建，Tauri配置负责将前端产物打包为桌面应用，并在各平台生成原生安装包。版本1.0.4引入了自动更新系统、改进的平台特定功能和增强的发布说明。
...
## 架构概览
下图展示了从开发到打包的关键流程：前端构建产物被Tauri作为静态资源加载；后端通过命令处理器与系统交互；最终生成多平台安装包。版本1.0.4引入了自动更新系统、改进的平台特定功能和增强的发布说明。
...
### Tauri 应用配置
- 产品信息：名称、版本、标识符统一管理，版本号更新至1.0.4。
...
### 单实例行为实现
**新增** 版本1.0.4引入了单实例控制功能，防止应用程序重复启动：
...
## 依赖关系分析
前端与后端通过Tauri配置建立紧密耦合：前端构建产物作为静态资源嵌入，后端通过命令通道与前端通信。版本1.0.4引入了自动更新系统和平台特定功能，进一步增强了整个系统的稳定性。Cargo与package.json分别管理后端与前端依赖，版本号保持一致以避免运行时冲突。
...
### 发布流程
- 版本管理：使用Git标签作为版本标识，自动生成对应版本号（1.0.4）
...
## 结论
本指南基于仓库现有配置，提供了IPSwitcher从开发到发布的完整路径。版本1.0.4引入了重要的新功能，包括增强的GitHub Actions工作流、平台特定的安装指导、自动更新系统、单实例行为、改进的Windows平台网络管理、增强的macOS激活功能和跨平台数据库支持。新增的GitHub Actions CI/CD工作流实现了完全自动化的多平台构建和发布流程，显著提升了开发效率和发布质量。增强的发布说明为用户提供了清晰的平台特定安装指导，而代码签名支持确保了应用的安全性。建议在正式发布前统一版本号、完善平台图标与最低系统版本设置，并充分利用CI/CD自动化流水线实现持续交付。
...
### 发布流程
- 版本管理：使用Git标签作为版本标识，自动提取版本号（1.0.4）
...
### 版本升级注意事项
- **版本号更新**：确保前端、后端和配置文件中的版本号保持一致（1.0.4）
